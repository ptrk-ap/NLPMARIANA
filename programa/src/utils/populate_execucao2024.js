...
async function autenticar(usuario, senha) {
  ...
  const options = {
    ...
    headers: {
      ...
      "User-Agent": "Mozilla/5.0 (compatible; populate_execucao2024/1.0)",
    },
  };
...
async function setupDatabase() {
  ...
  const hasTable = await knex.schema.hasTable("execucao2024");

  if (!hasTable) {
    await knex.schema.createTable("execucao2024", (table) => {
    ...
    console.log("✔ Tabela 'execucao2024' criada.");
  } else {
    console.log("✔ Tabela 'execucao2024' já existe.");
  }

  await knex("execucao2024").truncate();
...
async function salvarNoBanco(linhas) {
  ...
  for (let i = 0; i < linhas.length; i += BATCH_SIZE) {
    ...
    await knex("execucao2024").insert(lote);
...
async function populateExecucao2024() {
  console.log("=== Importador FlexSiafe AP → execucao2024 ===\n");
  ...
  console.log("Buscando dados da consulta 012356...");
...
module.exports = { populateExecucao2024 };
